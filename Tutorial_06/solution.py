...
def main():
    ...
    noise =  (255*np.random.normal(0,1,image.size)).astype(np.uint8)
    Image.fromarray(noise).save("output/noise.png","PNG")
    image_noisy = Image.fromarray(np.maximum(np.array(image),noise))
    ...
    image_guass_filter.save("output/image_guass.filter.png","PNG")
